...
# We optimize the gap function for different initial simplices to find the global minimum
# According to our tests, 10 random simplices are enough to heuristically find the largest gap
...
for _ in range(num_initial_guesses):
    ...
    result = minimize(objective, x0, method='nelder-mead', bounds=[(0.0, pi), (0.0, pi), (0, 2 * pi)], tol=1e-12)
